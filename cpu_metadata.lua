...
function p_cpu_metadata.dissector(buffer, pinfo, tree)
    ...
    if origEtherType == ethertype_ospf then
        -- Call the ARP dissector for the rest of the packet
        local ospf_dissector = Dissector.get("ospf")
        if ospf_dissector then
            ospf_dissector:call(buffer(5):tvb(), pinfo, tree)
        end
    end
